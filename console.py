                    ...
                    if value[0] == value[-1] == '"':
                        ...
                        try:
                            ...
                            try:
                                ...
                                continue
                    new_dict[key] = value
        if 'created_at' not in new_dict:
            new_dict['created_at'] = datetime.now().strftime(
                    '%Y-%m-%dT%H:%M:%S.%f')
        if 'updated_at' not in new_dict:
            new_dict['updated_at'] = datetime.now().strftime(
                    '%Y-%m-%dT%H:%M:%S.%f')
    ...
    def do_all(self, arg):
        ...
        if args and args[0] in HBNBCommand.classes:
            ...
            if cls_name in self.classes:
                obj_list = storage.all(self.classes[cls_name]).values()
            else:
                obj_list = []
        else:
